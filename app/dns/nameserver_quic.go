...
func (s *QUICNameServer) findSingleStackIPsForDomain(domain string, isIPv6 bool) ([]net.IP, time.Time, error) {
	var ipRecords map[string]*IPRecord
	if isIPv6 {
		ipRecords = s.ip6
	} else {
...
func (s *QUICNameServer) findDualStackIPsForDomain(domain string) ([]net.IP, time.Time, error) {
	// WTF
