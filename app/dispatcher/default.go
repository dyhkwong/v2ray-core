...
func (d *DefaultDispatcher) getLink(ctx context.Context, network net.Network, sniffing session.SniffingRequest) (*transport.Link, *transport.Link) {
	...
	var fdns dns.FakeDNSEngine
	fdnsFeat := core.MustFromContext(ctx).GetFeature((*dns.FakeDNSEngine)(nil))
	if fdnsFeat != nil {
		fdns = fdnsFeat.(dns.FakeDNSEngine)
	}
	fkr0, ok := fdns.(dns.FakeDNSEngineRev0)

	if ok && network == net.Network_UDP && sniffing.Enabled && shouldOverrideFakeDNS {
		opt = append(opt, pipe.OnTransmission(func(mb buf.MultiBuffer) buf.MultiBuffer {
			...
			for _, buffer := range mb {
				if buffer.Endpoint != nil {
					addr := buffer.Endpoint.Address
					if addr.Family().IsIP() && fkr0.IsIPInIPPool(addr) {
						domain := fkr0.GetDomainFromFakeDNS(addr)
						if len(domain) > 0 {
							buffer.Endpoint.Address = net.DomainAddress(domain)
							newError("override buffer.Endpoint: " + domain).WriteToLog(session.ExportIDToError(ctx))
						}
					}
				}
