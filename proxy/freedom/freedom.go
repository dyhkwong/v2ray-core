...
func (h *Handler) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	...
	requestDone := func() error {
		...
		if destination.Network == net.Network_TCP {
			...
			writer = NewPacketWriter(ctx, conn, h, UDPOverride)
		}
...
func NewPacketReader(conn net.Conn, UDPOverride net.Destination) buf.Reader { //nolint
	iConn := conn
...
func NewPacketWriter(ctx context.Context, conn net.Conn, h *Handler, UDPOverride net.Destination) buf.Writer { //nolint
	iConn := conn
	statConn, ok := iConn.(*internet.StatCouterConnection)
	if ok {
		iConn = statConn.Connection
