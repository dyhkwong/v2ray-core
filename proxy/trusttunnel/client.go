...
func (c *Client) setupHTTPTunnel(ctx context.Context, target string, dialer internet.Dialer, firstPayload []byte) (net.Conn, buf.MultiBuffer, error) {
	...
	connectHTTP2 := func(conn *h2Conn) (net.Conn, error) {
		pr, pw := io.Pipe()
		req.Body = pr
		var pErr error
	...
	connectHTTP3 := func(conn *h3Conn) (net.Conn, error) {
		pr, pw := io.Pipe()
		req.Body = pr
		var pErr error
		var wg sync.WaitGroup
	...
	if !c.config.Http3 {
		...
		c.cacheConnsLock.Unlock()
		if cachedConn != nil {
			conn := cachedConn.Value.(*h2Conn)
			if conn.h2Conn.CanTakeNewRequest() {
				proxyConn, err := connectHTTP2(conn)
				if err != nil {
					return nil, nil, err
				}
		...
		c.cacheConnsLock.Unlock()
		if cachedConn != nil {
			conn := cachedConn.Value.(*h3Conn)
			select {
			case <-conn.h3Conn.Context().Done():
			default:
				proxyConn, err := connectHTTP3(conn)
				if err != nil {
					return nil, nil, err
				}
				return proxyConn, nil, nil
