...
func NewPacketWriter(ctx context.Context, conn net.Conn, d *net.Destination, to *net.Destination, sockopt *internet.SocketConfig) buf.Writer {
	writer := &PacketWriter{
		ctx:     ctx,
		conn:    conn,
...
func (w *PacketWriter) WriteMultiBuffer(mb buf.MultiBuffer) error {
	for _, b := range mb {
		...
		var err error
		if b.Endpoint != nil && b.Endpoint.Address.Family().IsIP() {
			conn := w.conns[*b.Endpoint]
			if conn == nil {
				w.sockopt.BindAddress = b.Endpoint.Address.IP()
				w.sockopt.BindPort = uint32(b.Endpoint.Port)
				conn, _ = internet.DialSystem(w.ctx, *w.to, w.sockopt)
				if conn == nil {
					b.Release()
					continue
				}
				w.conns[*b.Endpoint] = conn
			}
