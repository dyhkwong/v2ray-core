...
func (c *Client) setupHTTPTunnel(ctx context.Context, target string, dialer internet.Dialer, firstPayload []byte, config *ClientConfig) (net.Conn, error) {
	...
	connectHTTP3 := func(rawConn net.Conn, quicConn *quic.Conn, h3clientConn *http3.ClientConn, readCounter, writeCounter stats.Counter) (net.Conn, error) {
		pr, pw := io.Pipe()
		req.Body = pr
	...
	if cachedConn != nil {
		...
		rawConn, quicConn, h3Conn := cachedConn.Value.(*h3Conn).rawConn, cachedConn.Value.(*h3Conn).quicConn, cachedConn.Value.(*h3Conn).h3Conn
		select {
		case <-h3Conn.Context().Done():
		default:
			proxyConn, err := connectHTTP3(rawConn, quicConn, h3Conn, readCounter, writeCounter)
			if err != nil {
				return nil, err
			}
	...
	quicConn, err := quic.Dial(ctx, packetConn, rawConn.RemoteAddr(),
		tlsSettings.GetTLSConfig(v2tls.WithNextProto("h3"), v2tls.WithDestination(c.serverAddress)),
		&quic.Config{
	...
	proxyConn, err := connectHTTP3(rawConn, quicConn, h3clientConn, readCounter, writeCounter)
	if err != nil {
		return nil, err
	}
